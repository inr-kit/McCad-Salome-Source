...
#include <math.h>
#include <time.h>
//#include <assert.h> //qiu stop the assert to abort the program
...
//qiu for exporting the class and function in 
#ifdef WIN32
...
class Standard_EXPORT tetgenio {
...
#ifdef TETLIBRARY
void Standard_EXPORT tetrahedralize(char *switches, tetgenio *in, tetgenio *out,
                    tetgenio *addin = NULL, tetgenio *bgmin = NULL);
...
//qiu 
//to avoid assertion to abort the program, which cause the SALOME to crash,
//we define a new assertion and just throw a make it exit

#ifdef WIN32 
#	define __ASSERT_VOID_CAST (void)
#	define assert(expr)   ((expr)	 ? 	__ASSERT_VOID_CAST(0) : throwExp())
		inline void throwExp(void) {
#			ifdef TETLIBRARY
				throw 11;
#			else
				exit(11);
...
#else //Linux
#	define __ASSERT_VOID_CAST (void)
	inline void __assert_fail (__const char *__assertion, __const char *__file,
               unsigned int __line)
	{
    printf("An input error was detected. The following assertion failed.\n");
    printf(__assertion);
    printf("; in file ");
    printf(__file);
