...
Standard_Boolean McCadBooleanOpt::Perform()
{
     ...
     BRepAdaptor_Surface BS(m_SplitFace, Standard_True);
     GeomAdaptor_Surface AdpSurf = BS.Surface();

     TopoDS_Face split_face = CrtExtSplitFace(AdpSurf);
...
/*
//from GEOMimpl_PartitionDriver.cxx
     TopTools_DataMapOfShapeShape aCopyMap,bCopyMap;
     TColStd_IndexedDataMapOfTransientTransient aMapTShapes,bMapTShapes;
     TopoDS_Shape aShape_i_copy;
     TNaming_CopyShape::CopyTool(m_InputSolid, aMapTShapes, aShape_i_copy);
     ...
     TopExp::MapShapes(m_InputSolid, aShape_i_inds);
     ...
     PrepareShapes(aShape_i_copy, aSimpleShapes);
     ...
     for (; aSimpleIter.More(); aSimpleIter.Next()) {
       ...
       if (ShapesMap.Add(aSimpleSh)) {
         Splitter.AddArgument(aSimpleSh);

       }
     }

     TopoDS_Shape bShape_i_copy;
     TNaming_CopyShape::CopyTool(split_face, bMapTShapes, bShape_i_copy);
     //if (aCopyTool.IsDone())
     ...
     //
     // fill aCopyMap for history
     TopTools_IndexedMapOfShape bShape_i_inds;
     TopTools_IndexedMapOfShape bShape_i_copy_inds;
     TopExp::MapShapes(split_face, bShape_i_inds);
     TopExp::MapShapes(bShape_i_copy, bShape_i_copy_inds);
     Standard_Integer mbInds = bShape_i_inds.Extent();
     for (Standard_Integer ie = 1; ie <= mbInds; ie++) {
       bCopyMap.Bind(bShape_i_inds.FindKey(ie), bShape_i_copy_inds.FindKey(ie));
     }
     //
     TopTools_ListOfShape bSimpleShapes;
     //PrepareShapes(aShape_i, aType, aSimpleShapes);
     PrepareShapes(bShape_i_copy, bSimpleShapes);
     TopTools_ListIteratorOfListOfShape bSimpleIter (bSimpleShapes);
     for (; bSimpleIter.More(); bSimpleIter.Next()) {
       const TopoDS_Shape& bSimpleSh = bSimpleIter.Value();
       if (!ShapesMap.Contains(bSimpleSh) && ToolsMap.Add(bSimpleSh)) {
         Splitter.AddTool(bSimpleSh);
       }
     }
*/
     Splitter.AddArgument(m_InputSolid);
     Splitter.AddTool(split_face);

//     Splitter.SetLimitMode(9);
//     Splitter.SetLimit(0);
     ...
     if (Splitter.ErrorStatus() == 0)
         ...
         cout <<"cut  with " << Splitter.ErrorStatus() <<endl;

     TopoDS_Shape shape = Splitter.Shape();

     if (!BRepAlgo::IsValid(shape)) {
       // 08.07.2008 added by skl during fixing bug 19761 from Mantis
       ShapeFix_ShapeTolerance aSFT;
       aSFT.LimitTolerance(shape, Precision::Confusion(),
     ...
     if (shape.ShapeType() == TopAbs_COMPSOLID || shape.ShapeType() == TopAbs_COMPOUND)
     {
         ...
         for (ex.Init(shape, TopAbs_SOLID); ex.More(); ex.Next())
         {
             ...
             m_ResultSolids->Append(tmpSol);
         }
     }
     else if (shape.ShapeType() == TopAbs_SOLID)
     {
         TopoDS_Solid tmpSol = TopoDS::Solid(shape);
         m_ResultSolids->Append(tmpSol);
     }
     else
     {
...
Standard_Boolean McCadBooleanOpt::SplitCyln(TopoDS_Face & theFace)
{
    ...
    try
    {
        ...
        //aDF.SetShapes(m_InputSolid, NegCylnSld);
        //aDF.Perform();
...
* @author  Lei Lu
***********************************************************************/
TopoDS_Face McCadBooleanOpt::CrtExtSplitFace(GeomAdaptor_Surface &AdpSurf)
{    
