...
void McCadConvexSolid::DeleteRepeatFace(Standard_Boolean bMergeDiscPnt)
{
    for (Standard_Integer i = 0; i < m_STLFaceList.size()-1; i++)
    {
        ...
        for (Standard_Integer j = i+1; j < m_STLFaceList.size(); j++)
        {
            ...
            if(pLeftFace->GetFaceNum() == pRightFace->GetFaceNum())
            {
                if (bMergeDiscPnt)
                {
                    //pLeftFace->AddPntList(pRightFace->GetDiscPntList());
                    //pLeftFace->UpdateBndBox(pRightFace->GetBndBox());
                    //pLeftFace->AddAuxFaces(pRightFace->GetAuxFaces());
                    //pLeftFace->AddMergedFaces(pRightFace);
...
//            {
//                for (Standard_Integer iAux = 0; iAux < pLeftFace->GetAuxFaces().size(); iAux++)
//                {
//                    McCadExtFace *pLeftAuxFace = pLeftFace->GetAuxFaces().at(iAux);
//                    for (Standard_Integer jAux = 0; jAux < pRightFace->GetAuxFaces().size(); jAux++)
//                    {
//                        McCadExtFace *pRightAuxFace = pRightFace->GetAuxFaces().at(jAux);
//                        if(pLeftAuxFace->GetFaceNum() == pRightAuxFace->GetFaceNum())
//                        {
//                            pRightFace->GetAuxFaces().erase(pRightFace->GetAuxFaces().begin()+jAux);
//                            delete pRightAuxFace;
//                            pRightAuxFace = NULL;
//                            jAux--;
//                        }
//                    }
//                }
//            }
          /*  else if (!pLeftFace->HaveAuxSurf() && pRightFace->HaveAuxSurf())
            {
                for (Standard_Integer iAux = 0; iAux < pRightFace->GetAuxFaces().size(); iAux++)
                {
                     McCadExtFace *pRightAuxFace = pRightFace->GetAuxFaces().at(iAux);
                     if(pRightAuxFace->GetFaceNum() == pLeftFace->GetFaceNum())
                     {
            ...
            else if(pLeftFace->HaveAuxSurf() && !pRightFace->HaveAuxSurf())
            {
                ...
                {
                    McCadExtFace *pLeftAuxFace = pLeftFace->GetAuxFaces().at(iAux);
                    if(pLeftAuxFace->GetFaceNum() == pRightFace->GetFaceNum())
                    {
                       m_STLFaceList.erase(m_STLFaceList.begin()+j);
                       delete pRightFace;
                       pRightFace = NULL;
    ...
    for (Standard_Integer i = 0; i < m_STLFaceList.size()-1; i++)
        {
            ...
            for (Standard_Integer j = 0; j < m_STLFaceList.size(); j++)
            {
                ...
                for (Standard_Integer k = 0; k < pRightFace->GetAuxFaces().size(); k++)
                {
                    ...
                    {
                        cout<<pLeftFace->GetFaceNum()<<" "<<pAuxFace->GetFaceNum()<<endl;
...
void McCadConvexSolid::DeleteRepeatAuxFace(McCadExtFace *& pLeftFace, McCadExtFace *& pRightFace,
                                           unsigned int &iErase, unsigned int &jErase)
{
    for (Standard_Integer i = 0; i < pLeftFace->GetAuxFaces().size(); i++)
    {
        ...
        for (Standard_Integer j = 0; j < pRightFace->GetAuxFaces().size(); j++)
        {
            ...
            if(pLeftAuxFace->GetFaceNum() == pRightAuxFace->GetFaceNum())
            {
                pRightFace->GetAuxFaces().erase(pRightFace->GetAuxFaces().begin()+j);
                delete pRightAuxFace;
                pRightAuxFace = NULL;
                j--;
            }
            /*else if(pLeftAuxFace->GetFaceNum() == pRightFace->GetFaceNum())
            {
                pLeftFace->GetAuxFaces().erase(pLeftFace->GetAuxFaces().begin()+i);
                delete pLeftAuxFace;
                pLeftAuxFace = NULL;
                i--;
            }*/
...
{
    return McCadGeomTool::GetVolume(*this);
}
